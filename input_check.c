...
/*   Updated: 2024/12/18 22:39:38 by rsaueia          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	is_empty_string(char *str)
{
	...
	while (str[i])
	{
		if (str[i] != ' ' || is_whitspace(str[i]))
			// If it finds any non space characters, it's not empty
			return (0);
		i++;
	}
	return (1);
	...
	while (str[i])
	{
		if (ft_strchr("|<>", str[i])) // is it a delim char?
		{
			if ((str[i] == '<' && str[i + 1] == '<') || (str[i] == '>' && str[i
					+ 1] == '>')) // here we check for heredoc and redirect
				i++;
...
/*int	quotes_check(char *str)
{
	...
	{
		if (str[i] == '\'')
			// Here you have to escape the character (didn't know that haha)
			single_quote++;
		else if (str[i] == '\"')
			// Not sure if I have to escape double quotes too,
