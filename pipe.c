...
/*   Updated: 2024/12/18 22:32:51 by rsaueia          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*static void    process_pipe(t_init_input *input_list, t_types *types,
	t_envp *env_list)
{
	char	*prompt;
	char	**args;
...
int	setup_pipeline(t_init_input *input_list, t_envp *env_list)
{
	...
	while (current)
	{
		...
		if (pid == 0) // Processo filho
		{
			...
			else if (current != input_list && current->next)
				//condição de verificação dos nós intermediários
			{
				dup2(current->fd_in, STDIN_FILENO);
				dup2(current->fd_out, STDOUT_FILENO);
				close(current->fd_in);
		...
		{
			if (current->fd_in != STDIN_FILENO)
				// Processo pai: gerenciar os pipes
				close(current->fd_in);
			if (current->fd_out != STDOUT_FILENO)
				close(current->fd_out);
