...
/*   Updated: 2024/12/20 23:37:30 by rsaueia          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	args_of_cmds(t_types *cmd)
{
	...
	while (cmd)
	{
		...
		else
		{
			...
			if (cmd && !is_delim(cmd->type) && cmd->prev
				&& is_redirects(cmd->prev->type)
				cmd->type = FLE;
		}
		if (cmd && !cmd->next && node_ref == 0 && !is_delim(cmd->type))
			// Marca o último argumento do comando como FLE (ex: cat Makefile)
			cmd->type = FLE;
...
void	args_of_cmds(t_types *cmd)
{
	...
	while (cmd)
	{
		if (node_ref == 1)
		{
			if (is_redirects(cmd->type))
			{
				if ((ft_strcmp(cmd->cmd, ">") == 0 || ft_strcmp(cmd->cmd,
							"<") == 0) && cmd->next->cmd)
					cmd->next->type = FLE;
