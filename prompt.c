...
/*   Updated: 2024/12/18 22:32:32 by rsaueia          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	process_shell_input(char *prompt_dup, t_init_input *input_list,
		t_envp *env_list)
{
...
static void	handle_prompt_input(char *prompt, char **prompt_dup,
...
/*void	prompt(char **envp)
{
	...
	while (1) // loop the shell.
	{
		...
		if (add_to_history(prompt)) // add the prompt to the history and go on
			prompt_dup = ft_strdup(prompt);
		//printf("prompt: [%s]\n", prompt);
		if (ft_strncmp(prompt, "exit", 4) == 0) //if the user types exit,
			the shell will exit.
		{
			int ret;
			int i;
			...
			exit_mini(input_list, prompt, prompt_dup, env_list);
				// exit the shell end clear the memory
			exit(ret);
		}
		else
		{
			if (!input_check(prompt_dup)) // check if the input is valid
			{
				...
				process_input(input_list, input_list->types, prompt_dup,
					env_list);
			}
			else
			{
